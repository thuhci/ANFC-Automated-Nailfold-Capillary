...
import cv2
import numpy as np
from Video_Process.nailfold_video_process.utils.affine_ransac import Ransac
from Video_Process.nailfold_video_process.utils.affine_transform import Affine
...
class Align():
    ...
    def warp_image(self, source, target, M, name):
        ''' WARP_IMAGE
        ...
        merge = np.uint8(warp)[50:400,100:600]
        # Show the result
        os.makedirs('./aligned',exist_ok=True)
        cv2.imwrite(f'./aligned/{name}.png',merge)
        return

    def align_image(self, name):
        ''' ALIGN_IMAGE
        ...
        for pair in fit_pos:
            s = kp_s[:,pair[0]]
            t = kp_t[:,pair[1]]
            dist.append(np.linalg.norm(s-t))
        dist = np.mean(np.array(dist))
        for pair in fit_pos:
            s = kp_s[:,pair[0]]
            t = kp_t[:,pair[1]]
            if np.linalg.norm(s-t) < dist+MAX_DISTANCE:
                if len(new_fit_pos) <= 3:
                    new_fit_pos.append(pair)
                else:
                    cnt = 0
                    # if one kp is connected to two kp, then leave it
                    for p in fit_pos:
                        if p[0] == pair[0] or p[1] == pair[1]:
                            cnt += 1
                    if cnt <= 1:
                        new_fit_pos.append(pair)
        if len(new_fit_pos) >= 3:   
            fit_pos = np.array(new_fit_pos)
        ...
        image = img_source.copy()
        for kp in kp_s_.transpose():
            image = cv2.circle(image.copy(), [int(p) for p in kp], 5, (255,255,255), 2)
        ax[0].imshow(image)
        image = img_target.copy()
        for kp in kp_t_.transpose():
            image = cv2.circle(image.copy(), [int(p) for p in kp], 5, (255,255,255), 2)
        ax[1].imshow(image)
        os.makedirs('./keypoint',exist_ok=True)
        plt.savefig(f'./keypoint/kp_{name}.png')
        
        # Compute the affine transformation matrix
        M = self.affine_matrix(kp_s, kp_t, fit_pos)

        # Warp the source image and display result
        self.warp_image(img_source, img_target, M, name)
